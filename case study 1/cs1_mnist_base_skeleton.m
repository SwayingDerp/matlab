...
for iter=1:max_iter
      ...
      for i=1:size(train,1);
          current_image = train(i, 1:end-1);
          centroids_pixels_only = centroids(:, 1:size(current_image, 2));
          [cluster_idx, min_dist] = assign_vector_to_centroid(current_image, centroids_pixels_only);
          train(i, end) = cluster_idx;
          total_cost = total_cost + min_dist^2;
      end
...
figure;
% FILL THIS IN!
plot(1:max_iter, cost_iteration, 'b-o', 'LineWidth', 2);
xlabel('Iteration');
ylabel('K-means Cost');
...
distances = zeros(num_centroids, 1);
    for j=1:num_centroids
        current_centroid = centroids(j, 1:length(data));
        differences = data - current_centroid;
...
for cluster_num = 1:K
    cluster_assignments = data(:, end);
    is_in_cluster = (cluster_assignments == cluster_num);
    cluster_pixels = data(is_in_cluster, 1:num_features);
    if size(cluster_pixels, 1) > 0
        new_centroids(cluster_num, :) = mean(cluster_pixels, 1);
    else
        random_image_index = randi(size(data, 1));
        new_centroids(cluster_num, :) = data(random_image_index, 1:num_features);
    end
end
end